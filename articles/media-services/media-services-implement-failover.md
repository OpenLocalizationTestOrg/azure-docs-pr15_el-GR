<properties 
    pageTitle="Εφαρμογή ανακατεύθυνσης ροής σενάριο | Microsoft Azure" 
    description="Αυτό το θέμα δείχνει πώς μπορείτε να υλοποιήσετε ανακατεύθυνσης ροής σενάριο." 
...
#<a name="implementing-failover-streaming-scenario"></a>Εφαρμογή ανακατεύθυνσης ροής σεναρίου

Αυτόν τον Οδηγό παρουσιάζει πώς μπορείτε να αντιγράψετε περιεχόμενο (αντικείμενα BLOB) από μία περιουσιακών στοιχείων σε ένα άλλο για να χειριστείτε πλεονασμού για ροή On Demand. Αυτό το σενάριο είναι χρήσιμη για τους πελάτες που θέλετε να ρυθμίσετε τους CDN ανακατεύθυνση μεταξύ των δύο κέντρα δεδομένων στην περίπτωση μιας μη διαθεσιμότητα σε ένα από τα κέντρα δεδομένων.
Αυτόν τον Οδηγό χρησιμοποιεί Microsoft Azure Media Services SDK, το Microsoft Azure Media Services REST API και Azure SDK χώρου αποθήκευσης για μια επίδειξη του τις παρακάτω εργασίες.

1. Ρύθμιση λογαριασμού Media Services στο "Κέντρο δεδομένων Α".
1. Αποστείλετε ένα αρχείο θυγατρική σε μια προέλευση περιουσιακών στοιχείων.
1. Κωδικοποίηση περιουσιακού στοιχείου σε bit πολλαπλό επιτόκιο MP4 αρχεία. 
1. Δημιουργήστε ένα προσδιοριστικό συσχετισμών Ασφαλείας μόνο για ανάγνωση του παγίου προέλευσης για να έχετε πρόσβαση για ανάγνωση στο κοντέινερ στο λογαριασμό χώρου αποθήκευσης που σχετίζεται με την προέλευση περιουσιακών στοιχείων.
1. Λάβετε το όνομα κοντέινερ του περιουσιακού στοιχείου προέλευσης από το προσδιοριστικό συσχετισμών Ασφαλείας μόνο για ανάγνωση δημιουργήσατε στο προηγούμενο βήμα. Χρειαζόμαστε αυτές τις πληροφορίες για την αντιγραφή αντικειμένων blob μεταξύ λογαριασμών χώρου αποθήκευσης (επεξηγούνται αργότερα στο θέμα).
1. Δημιουργήστε ένα προσδιοριστικό origin του παγίου που δημιουργήθηκε από την εργασία κωδικοποίησης. 

Στη συνέχεια, να χειριστείτε την ανακατεύθυνση:

1. Ρύθμιση λογαριασμού Media Services σε "B κέντρο δεδομένων".
1. Δημιουργήστε μια κενή περιουσιακών στοιχείων προορισμού στο λογαριασμό Media Services προορισμού.
1. Δημιουργήστε ένα προσδιοριστικό συσχετισμών Ασφαλείας εγγραφής για το πάγιο κενή προορισμού για να έχετε δικαιώματα εγγραφής στο κοντέινερ σε προορισμού λογαριασμού χώρου αποθήκευσης που σχετίζεται με το περιουσιακών στοιχείων προορισμού.
1. Χρησιμοποιήστε Azure SDK χώρου αποθήκευσης για να αντιγράψετε αντικείμενα BLOB (αρχεία περιουσιακών στοιχείων) μεταξύ του λογαριασμού χώρου αποθήκευσης προέλευσης στο "Κέντρο δεδομένων A" και του λογαριασμού χώρου αποθήκευσης προορισμού στο "B κέντρο δεδομένων" (αυτούς τους λογαριασμούς του χώρου αποθήκευσης είναι συσχετισμένες με τα στοιχεία που σας ενδιαφέρουν.)
1. Συσχετισμός αντικείμενα BLOB (αρχεία περιουσιακών στοιχείων) που έχουν αντιγραφεί στο κοντέινερ αντικειμένων blob προορισμού με παγίου προορισμού. 
1. Δημιουργήστε ένα προσδιοριστικό origin για περιουσιακού στοιχείου σε "B κέντρο δεδομένων" και καθορίστε το προσδιοριστικό αναγνωριστικό που δημιουργήθηκε για περιουσιακού στοιχείου σε "Κέντρο δεδομένων Α". 
1. Αυτό σας δίνει τη ροή διευθύνσεις URL όπου οι σχετικές διαδρομές από τις διευθύνσεις URL είναι το ίδιο (μόνο για τη βάση διαφέρουν διευθύνσεις URL). 
 
Στη συνέχεια, να χειρίζεται τις διακοπές, μπορείτε να δημιουργήσετε ένα CDN επάνω σε αυτές τις locators origin. 

Ισχύουν τα ακόλουθα θέματα:

- Η τρέχουσα έκδοση του SDK υπηρεσίες πολυμέσων δεν υποστηρίζει τη δημιουργία μιας προσδιοριστικό με καθορισμένο προσδιοριστικό αναγνωριστικό. Για να επιτύχετε αυτήν την εργασία, θα χρησιμοποιήσουμε Media Services REST API.
- Η τρέχουσα έκδοση του SDK υπηρεσίες πολυμέσων δεν υποστηρίζει μέσω προγραμματισμού παραγωγής πληροφορίες IAssetFile που θα συσχετίσετε ενός περιουσιακού στοιχείου με τα αρχεία περιουσιακών στοιχείων. Για να επιτύχετε αυτήν την εργασία, θα χρησιμοποιήσουμε το REST API CreateFileInfos Media Services. 
- Χώρος αποθήκευσης κρυπτογραφημένα Παγίων (AssetCreationOptions.StorageEncrypted) δεν υποστηρίζονται για αναπαραγωγή (εφόσον του κλειδιού κρυπτογράφησης θα είναι διαφορετικό σε δύο λογαριασμούς Media Services). 
- Εάν θέλετε να εκμεταλλευτείτε δυναμικής συσκευασία, πρώτα πρέπει να αποκτήσετε τουλάχιστον μία στην-ζήτηση ροής δεσμευμένη μονάδες. Για περισσότερες πληροφορίες, ανατρέξτε στο θέμα [Δυναμικά στοιχεία συσκευασία](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Μπορείτε να χρησιμοποιήσετε το Media Services [Εργαλείο αναπαραγωγής](http://replicator.codeplex.com/) ως εναλλακτική λύση για την εφαρμογή με μη αυτόματο τρόπο ένα σενάριο ροής ανακατεύθυνσης. Αυτό το εργαλείο σάς επιτρέπει να αναπαραγάγετε περιουσιακών στοιχείων σε δύο λογαριασμούς Media Services.

##<a name="prerequisites"></a>Προαπαιτούμενα στοιχεία
 
- Δύο λογαριασμούς Media Services σε μια νέα ή υπάρχουσα συνδρομή Azure. Δείτε [πώς μπορείτε να δημιουργήσετε ένα λογαριασμό υπηρεσίες πολυμέσων](media-services-portal-create-account.md).
- Λειτουργικά συστήματα: Windows 7, Windows 2008 R2 ή Windows 8.
- .NET framework διαίρεσης 4,5 ή .NET Framework 4.
- Visual Studio 2010 SP1 ή νεότερη έκδοση (Professional, Premium, Ultimate ή Express).
...
1. Χρησιμοποιήστε το Visual Studio για να δημιουργήσετε μια νέα λύση που περιέχει το έργο εφαρμογής κονσόλας C#. Πληκτρολογήστε HandleRedundancyForOnDemandStreaming για το όνομα και, στη συνέχεια, κάντε κλικ στο κουμπί OK.
1. Δημιουργήστε το φάκελο SupportFiles στο ίδιο επίπεδο με το αρχείο έργου HandleRedundancyForOnDemandStreaming.csproj. Κάτω από το φάκελο SupportFiles Δημιουργήστε τους φακέλους OutputFiles και MP4Files. Αντιγραφή αρχείου .mp4 στον φάκελο MP4Files (σε αυτό το παράδειγμα χρησιμοποιείται το αρχείο BigBuckBunny.mp4). 
...
1. Προσθέστε άλλες αναφορές που απαιτούνται για αυτό το έργο: System.Configuration, System.Runtime.Serialization και System.Web.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Προσθήκη κώδικα που χειρίζεται πλεονασμού για ροή On Demand.



1. Προσθέστε τα παρακάτω πεδία επίπεδο κλάσης για το εκπαιδευτικό πρόγραμμα.
...
1. Αντικατάσταση τον ορισμό προεπιλεγμένες κύριες μέθοδο με το παρακάτω:
...
1. Μέθοδος ορισμούς που ονομάζονται από το κύριο ορίζονται κάτω από το στοιχείο.
...
Τώρα, μπορείτε να χρησιμοποιήσετε Διαχείριση κίνηση σε αιτήσεις δρομολόγηση μεταξύ της κέντρα δεδομένων δύο και, επομένως, ανακατεύθυνσης στην περίπτωση οποιαδήποτε διακοπές.
