...
Αυτό το πρόγραμμα εκμάθησης σάς βοηθά να αναπτύξετε μια απλή εφαρμογή web HTML + CSS σε [Azure εφαρμογής υπηρεσίας](../app-service/app-service-value-prop-what-is.md).
...
- Ανάπτυξη HTML και CSS σε αυτήν.
- Ανατρέξτε στο θέμα εκτέλεση ζωντανή παραγωγή τις σελίδες σας.
- Ενημερώστε το περιεχόμενό σας με τον ίδιο τρόπο όπως θα κάνατε [push δεσμεύεται Git](https://git-scm.com/docs/git-push).
...
## <a name="deploy-a-simple-html-site"></a>Ανάπτυξη μια απλή τοποθεσία HTML
...
4. Αναπτύξτε το δείγμα κώδικα για την εφαρμογή του Azure όπως κάνατε push οποιονδήποτε κωδικό με Git. Όταν σας ζητηθεί, χρησιμοποιήστε τον κωδικό πρόσβασης που έχετε ρυθμίσει προηγουμένως.
    ...
    Εάν χρησιμοποιείτε ένα από τα πλαίσια γλώσσα, θα βλέπετε διαφορετικό αποτέλεσμα. Αυτό συμβαίνει επειδή `git push` όχι μόνο τοποθετεί κώδικα στο Azure, αλλά ενεργοποιεί επίσης εργασίες ανάπτυξης στο το μηχανισμό ανάπτυξης. Εάν έχετε οποιοδήποτε package.json (Node.js) ή requirements.txt (Python) αρχεία στο τη ριζική έργου (αποθετήριο) ή εάν έχετε ένα αρχείο packages.config στο έργο σας ASP.NET, η δέσμη ενεργειών ανάπτυξης επαναφέρει τα απαιτούμενα πακέτα για εσάς. Μπορείτε επίσης να [ενεργοποιήσετε την επέκταση σύνθεσης](web-sites-php-mysql-deploy-use-git.md#composer) για την αυτόματη επεξεργασία composer.json αρχείων στην εφαρμογή PHP.
